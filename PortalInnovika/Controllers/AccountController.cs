...
namespace PortalInnovika.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public string GetMensajeInhabil()
        {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (cliente != null && cliente.Estatus.Trim() == "ALTA")
            {
                ...
                {
                    //string msgPersonalizado = GetMensajePersonalizado();
                    //string msgInhabil = GetMensajeInhabil();

                    //return RedirectToAction("Dashboard", "Home", new { msgPersonalizado, msgInhabil });
        ...
        //[NRB: 04-06-2014] RESET DE PASSWORD
        //
        // GET: /Account/PassReset
        [AllowAnonymous]
        public ActionResult PassReset(string username)
        {
            var token = WebSecurity.GeneratePasswordResetToken(username);
            WebSecurity.ResetPassword(token, "sistema");
            return RedirectToAction("Login", "Account");
        }
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    //var oldId = (from i in db.Users select i.UserId).FirstOrDefault();
                    var oldId = (from i in db.Users where i.ClienteERP == model.ClienteERP select i.UserId).FirstOrDefault();
                    WebSecurity.CreateUserAndAccount(model.UserName, model.Password, new { ClienteERP = model.ClienteERP, 
                                                                                           NombreCompleto = model.NombreCompleto,
                                                                                           Email = model.Email,
                                                                                           AgenteServicio = model.AgenteServicio,
                                                                                           PuedeExpress = model.PuedeExpress,
                                                                                           FleteraId = model.FleteraId,
                                                                                           FleteAutomatico = model.FleteAutomatico,
                                                                                           ClienteMuebles = model.ClienteMuebles,
                                                                                           ZonaImpuesto = model.ZonaImpuesto,
                                                                                           ListaPreciosEsp = model.ListaPreciosEsp,
                                                                                           UserOld = oldId
                    }, 
                false);
